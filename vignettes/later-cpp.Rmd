...
You can call `later::later` from C++ code in your own packages, to cause your own C-style functions to be called back. This is
safe to call from either the main R thread or a different thread; in both cases, your callback will be invoked from the main R
thread.

To use the C++ interface with the `cpp4r` package, follow these steps (it is analogous with
[cpp11](https://cran.r-project.org/package=cpp11)):

* Add `later` to your `DESCRIPTION` file under both `LinkingTo` and `Imports`, and add `cpp4r` under `LinkingTo`:
```
LinkingTo: cpp4r, later
Imports: later
```

* Make sure your package's R file registers the dynamic library and imports `later`. If your package uses roxygen2, add the
following to e.g. `R/mypkg-package.R`:
```
...
* Add `#include <cpp4r.hpp>` and `#include <later_api.h>` to the top of each C++ file that uses the below APIs.

You can simplify these steps a bit by creating a package skeleton with `cpp4r::pkg_template("mypkg")`.
...
that will be passed to the function when it's called back. And the third argument is the number of seconds to wait (at a minimum)
before invoking. In all cases, the function will be invoked on the R thread, when no user R code is executing.

## Background tasks

This package also offers a higher-level C++ helper class called `later::BackgroundTask`, to make it easier to execute tasks on a
...
point; you're responsible for providing the actual code that executes on the background thread, as well as code that executes on
...
It's critical that the code in your `execute` method not mutate any R data structures, call any R code, or cause any R
...
To run the task, `new` up your subclass and call `begin()`, e.g. `(new MyTask(vec))->begin()`. There's no need to keep track of
...
It's not very useful to execute tasks on background threads if you can't get access to the results back in R. We'll soon be
introducing a complementary R package that provides a suitable "promise" or "future" abstraction.
